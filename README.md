...
- **v1.1.1 报告路径修补**：回复不再半句中断、报告生成有进度、卡片含结论/依据/风险/下一步且与分析正文并存、表格正常渲染。详见 `docs/RELEASE-NOTES-1.1.1.md`。
- **v1.1.0 顾问能力**：空对话仪表盘、首次启动画像向导、结构化报告卡与研报夹、自选股及自选新闻订阅。详见 `docs/RELEASE-NOTES-1.1.0.md`。
- **搜股票**：按名称联想（如「茅台」），不再每次拉取全市场代码表。
- **图表与回测展示**：对日线类工具结果可内嵌 **K 线**（红涨绿跌）；`run_backtest` 成功返回时展示 **累计收益曲线**。
- **快捷回复**：助手可在回复末尾附带 **`FIN_AGENT_CHOICES_JSON`**，客户端解析为可点击选项。
