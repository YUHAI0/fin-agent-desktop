...
- [x] **Step 1: 创建 `datasources/tushare_provider.py`**
...
- [x] **Step 1: 在 `python/fin-agent/requirements.txt` 中加入 akshare**
...
- [x] **Step 3: 创建 `datasources/akshare_provider.py`**
...
- [x] **Step 1: 写入 `datasources/__init__.py`**
...
- [x] **Step 1: 创建 `tools/market_tools.py`**
...
- [x] **Step 2: 删除 `tushare_tools.py` 中六个核心函数的定义**
...
- [x] **Step 3: 在 `tushare_tools.py` 顶部导入 market_tools**
...
- [x] **Step 4: 给全部 Tushare 专属函数加 `@requires_tushare`**

对下列函数逐个在 `def` 上一行加 `@requires_tushare`：

`get_moneyflow`、`get_hsgt_top10`、`get_limit_list`、`get_top_list`、`get_forecast`、`get_concept_detail`、`get_long_tail_stocks`、`screen_stocks`、`get_hk_stock_basic`、`get_hk_daily_price`、`get_us_stock_basic`、`get_us_daily_price`、`get_hk_realtime_price`、`get_us_realtime_price`、`get_etf_basic`、`get_etf_daily_price`、`get_cb_basic`、`get_cb_daily_price`、`get_futures_basic`、`get_futures_daily_price`、`get_macro_gdp`、`get_macro_cpi`、`get_macro_m2`、`get_macro_interest_rate`、`get_global_index_comparison`

不加装饰器的：`get_current_time`、`reset_core_config`（不涉及行情数据）。

- [x] **Step 5: 把 `execute_tool_call` 中六个核心工具改指向 market_tools**
...
- [x] **Step 2: 在 `_get_system_content` 开头计算可用性说明**
...
- [x] **Step 3: 新增 `_get_datasource_note` 方法**
...
- [x] **Step 4: 把 `datasource_note` 插入返回的提示串**
...
- [x] **Step 1: `Config.validate()` 只在数据源为 tushare 时要求 Token**
...
- [x] **Step 4: 在 `handleSubmit` 的 config 对象中提交该字段**
...
- [x] **Step 5: 在 Tushare Token 区块之前插入数据源下拉**
...
- [x] **Step 6: Tushare Token 输入框去掉 required 并补说明**
...
- [x] **Step 1: 在 `pyinstaller_cmd` 中追加排除参数**
...
- [x] **Step 1: 创建 `python/fin-agent/fin_agent/session_store.py`**
...
- [x] **Step 1: 在 `python/api.py` 顶部导入 session_store**
...
- [x] **Step 2: 在 `_handle_chat_stream` 中解析 session_id 并选择 agent**
...
- [x] **Step 1: 删除 clearChatHistory 的全部 14 处调用**
...
- [x] **Step 4: 在发送 /chat 时带上 session_id**
...
- [x] **Step 1: 在 `src/preload/index.ts` 的 `api` 对象中追加方法**
...
- [x] **Step 3: 在标题栏与消息区之间插入标签栏，并在最外层容器内挂载抽屉**
...
- [x] **Step 5: 确认无需在 ChatView 中手动持久化**
...
- [x] **Step 1: 移除 schedule 导入，新增所需导入**
...
- [x] **Step 3: 用朴素循环替换 `run_scheduler`**
...
- [x] **Step 3: 在 `handleSubmit` 的 config 对象中提交**
...
- [x] **Step 4: 在"系统"区块的开机自启项之后插入两个表单项**
...
- [x] **Step 1: 替换组合级操作以外的旧方法，写入新的方法集**
...
- [x] **Step 1: 删除模块级全局实例，四个函数改为每次调用新建**
...
- [x] **Step 1: 在 `python/api.py` 导入 PortfolioManager**
...
- [x] **Step 1: 主进程注册 8 个 IPC handler**
