...
from fin_agent.tools.news_tools import (
    NEWS_TOOLS_SCHEMA,
    create_news_subscription,
    delete_news_subscription,
...
NEWS_TOOL_FUNCTIONS = {
    ...
    "update_news_subscription": update_news_subscription,
    "pause_news_subscription": pause_news_subscription,
    "enable_news_subscription": enable_news_subscription,
    "delete_news_subscription": delete_news_subscription,
    "refresh_news": refresh_news,
}


def _invalid_tool_arguments(tool_name, message):
    if tool_name in NEWS_TOOL_FUNCTIONS:
        return json.dumps(
            {"ok": False, "error": f"新闻工具参数错误：{message}"},
            ensure_ascii=False,
            indent=2,
        )
    return f"Error: Invalid tool arguments ({message})."
...
def get_concept_detail(concept_name=None, ts_code=None):
    ...
    try:
        ...
        def _load_concept_catalog():
            # Tushare concept 接口必须指定 src，否则返回「请指定正确的接口名」
            last_err = None
            for src in ("ts", "sina"):
                try:
            ...
            if last_err:
                raise last_err
            raise RuntimeError("概念分类列表为空")

        # If searching for what concepts a stock belongs to
        if ts_code:
            df = pro.concept_detail(ts_code=ts_code)
            if df.empty:
                return f"未找到股票 {ts_code} 的概念板块数据。"
            return df.to_json(orient='records', force_ascii=False)

        # If searching for stocks in a concept
        if concept_name:
            concepts = _load_concept_catalog()
            name_series = concepts['name'].astype(str)
            matched = concepts[name_series.str.contains(str(concept_name), na=False, regex=False)]

            if matched.empty:
                return f"未找到与「{concept_name}」匹配的概念板块。"
            ...
            if df.empty:
                 return f"概念「{concept_name}」下未找到成分股（ID: {concept_id}）。"
            return df.to_json(orient='records', force_ascii=False)

        return "错误：请提供 concept_name 或 ts_code 参数。"

    except Exception as e:
        msg = str(e)
        if "请指定正确的接口名" in msg:
            return (
                "查询概念板块失败：Tushare 概念接口调用异常。"
                "请确认 Token 有效且具备概念股相关接口权限（通常需 300 积分以上）。"
            )
        return f"查询概念详情失败：{msg}"
                ...
                try:
                    ...
                    if isinstance(val, dict):
                        ...
                        return _invalid_tool_arguments(
                            tool_name, "参数必须是 JSON 对象"
                        )
                except:
                    return _invalid_tool_arguments(
                        tool_name, "无法解析 JSON 参数"
                    )
    
    if arguments is None:
        arguments = {}
    if not isinstance(arguments, dict):
        return _invalid_tool_arguments(tool_name, "参数必须是对象")

    if tool_name in NEWS_TOOL_FUNCTIONS:
        function = NEWS_TOOL_FUNCTIONS[tool_name]
        try:
            inspect.signature(function).bind(**arguments)
        except TypeError as exc:
            return _invalid_tool_arguments(tool_name, str(exc))
